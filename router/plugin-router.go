...
func productionPluginRouteHandlers(generation *jsplugin.RoutingGeneration, binding jsplugin.RouteBinding) []gin.HandlerFunc {
	...
	return []gin.HandlerFunc{
		...
		middleware.RelayAutoBanClientMetrics(),
		middleware.RelayUserAgentBlacklist(),
		middleware.SystemPerformanceCheck(),
		middleware.UserRequestRateLimit(),
	...
	for _, route := range plugin.Meta.Routes {
		for _, staticRoute := range b.staticRoutes {
			if routeIntersectsStaticRoute(route.Path, staticRoute.Path) {
				// Existing provider-native routes keep their built-in adapters.
				// Their plugins remain available through generic and host protocols.
				legacyOwner := map[string]string{
					"/suno/submit/:action": "sunoapi", "/suno/fetch": "sunoapi", "/suno/fetch/:id": "sunoapi",
					"/kling/v1/videos/text2video": "kling", "/kling/v1/videos/image2video": "kling",
					"/kling/v1/videos/text2video/:task_id": "kling", "/kling/v1/videos/image2video/:task_id": "kling",
					"/jimeng/": "jimeng",
				}[staticRoute.Path]
				if legacyOwner == plugin.Meta.Key && route.Method == staticRoute.Method &&
					(route.Path == staticRoute.Path || route.Path == "/suno/fetch/:task_id" && staticRoute.Path == "/suno/fetch/:id") {
					continue
				}
