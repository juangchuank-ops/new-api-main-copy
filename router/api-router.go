...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.GET("/model-availability", middleware.HeaderNavModuleAuth("modelAvailability"), controller.GetModelAvailability)
		apiRouter.GET("/rankings", middleware.HeaderNavModuleAuth("rankings"), controller.GetRankings)
		apiRouter.GET("/rankings/availability", middleware.HeaderNavModuleAuth("rankings"), controller.GetRankingAvailability)
		apiRouter.GET("/rankings/security", middleware.HeaderNavModuleAuth("rankings"), controller.GetRankingSecurity)
		...
		apiRouter.POST("/oauth/complete", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, controller.CompleteOAuthRegistration)
		...
		selfRoute.Use(middleware.UserOrNewApiUserAuth()) // 支持 token 或 New-Api-User header
		{
			...
			gameRoute := selfRoute.Group("/game")
			{
				...
				// 邮件发送
				selfRoute.POST("/send-email", controller.SendEmailToSelf)
			...
			// Check-in routes - 使用 NewApiUserAuth，只需要 New-Api-User header
			checkinRoute := userRoute.Group("/")
			checkinRoute.Use(middleware.NewApiUserAuth())
			{
				checkinRoute.GET("/checkin", controller.GetCheckinStatus)
				checkinRoute.POST("/checkin", middleware.TurnstileCheck(), controller.DoCheckin)
			}
		...
		// Auto Sync 配置与状态路由
		autoSyncRoute := apiRouter.Group("/auto_sync")
		autoSyncRoute.Use(middleware.RootAuth())
		{
			autoSyncRoute.GET("/price", controller.GetAutoPriceSyncStatus)
			autoSyncRoute.PUT("/price", controller.UpdateAutoPriceSyncConfig)
			autoSyncRoute.GET("/model", controller.GetAutoModelSyncStatus)
			autoSyncRoute.PUT("/model", controller.UpdateAutoModelSyncConfig)
		}
		...
		channelRoute.Use(middleware.AdminAuth())
		{
			...
			// P1b-2: client identity version lookup (admin only, old middleware)
			channelRoute.GET("/client-identity/versions", controller.GetClientIdentityVersions)
		...
		apiRouter.GET("/invitation-code/check", middleware.CriticalRateLimit(), controller.CheckInvitationCode)

		userInvitationCodeRoute := apiRouter.Group("/user/invitation-code")
		userInvitationCodeRoute.Use(middleware.UserAuth())
		{
			userInvitationCodeRoute.GET("/", controller.GetUserInvitationCodes)
			userInvitationCodeRoute.GET("/available", controller.GetUserAvailableInvitationCodeKeys)
			userInvitationCodeRoute.POST("/batch-delete", controller.DeleteUserInvitationCodeBatch)
			userInvitationCodeRoute.DELETE("/used", controller.DeleteUserUsedInvitationCodes)
			userInvitationCodeRoute.DELETE("/:id", controller.DeleteUserInvitationCode)
		}
	...
	logRoute.DELETE("/", middleware.RootAuth(), controller.DeleteHistoryLogs)
		...
		apiRouter.DELETE("/system-info/stale-instances", middleware.AdminAuth(), controller.DeleteStaleSystemInstances)
		apiRouter.DELETE("/system-info/instances/:node_name", middleware.AdminAuth(), controller.DeleteStaleSystemInstance)
