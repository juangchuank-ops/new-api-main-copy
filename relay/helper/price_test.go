...
func TestFixedPricePreConsumeAndRealtimeRejection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"HTTP charges once", types.RelayFormatOpenAI, 0, false, false},
		{"Realtime rejects even unselected fixed branch", types.RelayFormatOpenAIRealtime, 50000, true, false},
		{"legacy tasks reject fixed expressions", types.RelayFormatOpenAI, 0, true, true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.perCall {
				...
				require.ErrorContains(t, err, "legacy task")
