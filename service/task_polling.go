...
var GetTaskPluginAdaptorFunc func(key string) TaskPollingAdaptor

func taskPollingAdaptor(platform constant.TaskPlatform, tasks map[string]*model.Task) TaskPollingAdaptor {
	if GetTaskPluginAdaptorFunc != nil {
		for _, task := range tasks {
			if task != nil && task.PrivateData.Execution != nil && task.PrivateData.Execution.TaskPlugin != nil && task.PrivateData.Execution.TaskPlugin.Key != "" {
				return GetTaskPluginAdaptorFunc(task.PrivateData.Execution.TaskPlugin.Key)
			}
		}
	}
	if platform == constant.TaskPlatformMidjourney || GetTaskAdaptorFunc == nil {
		return nil
	}
...
func RunTaskPollingOnce(ctx context.Context, report func(processed, total int)) TaskPollSummary {
	...
	type pollingIdentity struct {
		platform  constant.TaskPlatform
		pluginKey string
	}
	platformTask := make(map[pollingIdentity][]*model.Task)
	for _, t := range allTasks {
		identity := pollingIdentity{platform: t.Platform}
		if execution := t.PrivateData.Execution; execution != nil && execution.TaskPlugin != nil {
			identity.pluginKey = execution.TaskPlugin.Key
		}
		platformTask[identity] = append(platformTask[identity], t)
	}
	...
	for platform, tasks := range platformTask {
		...
		DispatchPlatformUpdate(ctx, platform.platform, taskChannelM, taskM)
	}
...
func DispatchPlatformUpdate(ctx context.Context, platform constant.TaskPlatform, taskChannelM map[int][]string, taskM map[string]*model.Task) {
	...
	adaptor := taskPollingAdaptor(platform, taskM)
	if adaptor == nil {
		// Legacy MJ has its own poller; a plugin named mj is still supported.
		return
	}
