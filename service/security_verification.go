...
func BindVerificationOperation(operation VerificationOperation) (VerificationBinding, error) {
	...
	case VerificationScopePasskeyDelete:
		if len(fields) == 0 {
			// The singular legacy endpoint removes all credentials.
			normalized = struct{}{}
			break
		}
		var context PasskeyDeleteContext
		if len(fields) != 1 || common.Unmarshal(fields["credential_id"], &context.CredentialID) != nil || context.CredentialID <= 0 {
			return VerificationBinding{}, ErrVerificationContextInvalid
		}
		normalized = context
