...
type TopUp struct {
	...
	ProductType     string  `json:"product_type" gorm:"type:varchar(50);default:'topup'"`
}
...
func RechargeEpay(tradeNo string, actualPaymentMethod string, callerIp string) (alreadyDone bool, err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if topUp.ProductType == "invitation_code" {
			if genErr := GenerateInvitationCodesForUser(topUp.UserId, int(topUp.Amount)); genErr != nil {
				return fmt.Errorf("生成邀请码失败: %w", genErr)
			}
	...
	if isInvitationCode {
		common.SysLog(fmt.Sprintf("易支付购买邀请码成功 trade_no=%s user_id=%d count=%d money=%.2f", topUp.TradeNo, topUp.UserId, topUp.Amount, topUp.Money))
		RecordTopupLog(topUp.UserId, fmt.Sprintf("购买邀请码成功，数量: %d，支付金额：%f", topUp.Amount, topUp.Money), callerIp, topUp.PaymentMethod, PaymentProviderEpay)
		return false, nil
	}
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	// 事务外记录日志，避免阻塞
	if productType == "invitation_code" {
		RecordTopupLog(userId, fmt.Sprintf("管理员补单成功（邀请码），支付金额：%f", payMoney), callerIp, paymentMethod, "admin")
	} else {
