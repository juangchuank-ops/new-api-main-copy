...
func (mi *Model) Insert() error {
	return metadataTransaction(func(tx *gorm.DB) error {
		// Scheduled imports may race a manual create. Reuse the active record
		// without overwriting its metadata, as the legacy import contract requires.
		var existing Model
		if err := tx.Where("active_name = ?", mi.ModelName).First(&existing).Error; err == nil {
			*mi = existing
			return nil
...
func (mi *Model) Update() error {
	return metadataTransaction(func(tx *gorm.DB) error {
		...
		return tx.Model(&Model{}).Where("id = ?", mi.Id).
			Select("model_name", "active_name", "description", "icon", "tags", "vendor_id", "endpoints", "status", "sync_official", "name_rule", "updated_time").Updates(mi).Error
	})
}
...
func DeleteModelMetadata(ids []int, removeFromChannels, removePricing bool) (ModelDeleteResult, error) {
	...
	deleteRecords := func(tx *gorm.DB) error {
		...
		if len(records) != len(modelIDs) {
			// Repeating a metadata-only single delete remains harmless. Requests
			// that also change channels or pricing still require fresh selection.
			if len(records) == 0 && len(modelIDs) == 1 && !removeFromChannels && !removePricing {
				return nil
			}
		...
		if err := tx.Model(&Model{}).Where("id IN ?", modelIDs).Update("active_name", nil).Error; err != nil {
			return err
		}
