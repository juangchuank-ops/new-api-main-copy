...
// ResolveNewUserRole returns the role a new user should receive.
// If no root user exists in the system yet, the first user is promoted
// to RoleRootUser so the system always has an administrator after the
// first registration. Subsequent users receive RoleCommonUser.
func ResolveNewUserRole() int {
	if !RootUserExists() {
