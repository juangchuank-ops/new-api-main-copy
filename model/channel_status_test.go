...
func TestUpdateChannelStatusPersistsMultiKeyState(t *testing.T) {
	...
	assert.False(t, changed, "one disabled key must not announce the entire channel as disabled")
	...
	assert.True(t, UpdateChannelStatus(channel.Id, "key-b", common.ChannelStatusAutoDisabled, "second key failed"))
	require.NoError(t, DB.First(&stored, channel.Id).Error)
...
func TestChannelStatusUpdatePreservesActivePollingCursor(t *testing.T) {
	setupChannelStatusTest(t)
	channel := Channel{
		Name: "polling-status", Key: "key-a\nkey-b", Status: common.ChannelStatusEnabled,
	...
	assert.False(t, UpdateChannelStatus(channel.Id, "key-a", common.ChannelStatusAutoDisabled, "key failed"))
	var stored Channel
	require.NoError(t, DB.First(&stored, channel.Id).Error)
	assert.Equal(t, 1, stored.ChannelInfo.MultiKeyPollingIndex)
	current, err := CacheGetChannel(channel.Id)
	require.NoError(t, err)
	key, index, apiErr := current.GetNextEnabledKey()
