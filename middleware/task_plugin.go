...
const contextKeyTaskPluginEndpointGeneration = "task_plugin_endpoint_generation"

// TaskPluginEndpointCandidates lets the normal channel selector choose between
// legacy channels and explicitly bound plugin channels before any JS decoding.
// The chosen generation stays pinned through selection and request execution.
func TaskPluginEndpointCandidates() gin.HandlerFunc {
...
func PinTaskPluginEndpoint() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if channelSelected {
			...
			generation, _ = selectedGeneration.(*pluginruntime.RoutingGeneration)
		}
		if generation == nil {
			c.Next()
			return
...
// deterministic parser pinned for the selected channel.
	...
	return func(c *gin.Context) {
		...
		if _, selected := c.Get(contextKeyTaskPluginEndpointGeneration); selected {
			if pin, found, _ := service.GetChannelConstraints(c).ResolvedPin(); found && pin.ChannelId != common.GetContextKeyInt(c, constant.ContextKeyChannelId) {
				...
				// selection. Reuse distribution's identity and authorization
				// checks before generating relay metadata for the first attempt.
				Distribute()(c)
				return
			}
		}
