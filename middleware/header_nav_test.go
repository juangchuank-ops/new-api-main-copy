...
func TestHeaderNavModuleAuthAllowsCommonUserForModelAvailability(t *testing.T) {
	raw := `{"modelAvailability":{"enabled":true,"requireAuth":true}}`
	withHeaderNavModules(t, raw)

	recorder := performHeaderNavRequest(t, HeaderNavModuleAuth("modelAvailability"), true)
