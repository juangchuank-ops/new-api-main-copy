...
func TestGPT6AstraBuiltinBilling(t *testing.T) {
	...
	t.Run("raw CAS options exclude display-only defaults", func(t *testing.T) {
		...
		assert.Equal(t, map[string]string{"billing_setting.billing_mode": "", "billing_setting.billing_expr": ""}, found)
		assert.Equal(t, "tiered_expr", billing_setting.GetBillingModeCopy()["gpt-6-astra"])
		assert.Equal(t, expression, billing_setting.GetBillingExprCopy()["gpt-6-astra"])
	})
