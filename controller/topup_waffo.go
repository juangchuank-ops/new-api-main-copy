...
func RequestWaffoAmount(c *gin.Context) {
	...
	// Invitation code: price based on count × InvitationCodePrice
...
func RequestWaffoPay(c *gin.Context) {
	...
	goodsInfo := buildWaffoTopUpGoodsInfo(amount)
	if req.ProductType == "invitation_code" {
		goodsInfo.GoodsName = fmt.Sprintf("Invitation Codes x%d", amount)
	}
...
func handleWaffoPayment(c *gin.Context, wh *core.WebhookHandler, result *core.PaymentNotificationResult) {
	...
	if topUp != nil && topUp.ProductType == "invitation_code" {
		if topUp.Status != common.TopUpStatusPending {
			...
			logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo 邀请码订单已处理 trade_no=%s status=%s client_ip=%s", merchantOrderId, topUp.Status, c.ClientIP()))
			sendWaffoWebhookResponse(c, wh, true, "")
			return
		}
		...
		if err := topUp.Update(); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo 更新邀请码订单状态失败 trade_no=%s error=%q", merchantOrderId, err.Error()))
		}
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo 邀请码购买成功 trade_no=%s count=%d client_ip=%s", merchantOrderId, topUp.Amount, c.ClientIP()))
		sendWaffoWebhookResponse(c, wh, true, "")
		return
	}
