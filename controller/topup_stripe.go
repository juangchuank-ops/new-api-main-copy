...
type StripePayRequest struct {
	...
	// ProductType discriminates "topup" (default) from "invitation_code".
	ProductType string `json:"product_type,omitempty"`
	// Count is the number of invitation codes to buy (1-100, only used when ProductType is "invitation_code").
...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	if req.ProductType == "invitation_code" {
		effectiveAmount = int64(req.Count)
		chargedMoney = common.InvitationCodePrice * float64(req.Count)
	} else if rejectInvalidCreditedQuota(c, id,
	...
	payLink, err := genStripeLink(referenceId, user.StripeCustomer, user.Email, effectiveAmount, req.SuccessURL, req.CancelURL)
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Stripe 创建 Checkout Session 失败 user_id=%d trade_no=%s amount=%d error=%q", id, referenceId, effectiveAmount, err.Error()))
	...
	if req.ProductType == "invitation_code" {
		topUp.ProductType = "invitation_code"
	}
	err = topUp.Insert()
	if err != nil {
...
func fulfillOrder(ctx context.Context, event stripe.Event, referenceId string, customerId string, callerIp string) {
	...
	// Check if this is an invitation code purchase — if so, generate codes and skip quota credit.
	topUp := model.GetTopUpByTradeNo(referenceId)
	if topUp != nil && topUp.ProductType == "invitation_code" {
		...
		if err := topUp.Update(); err != nil {
			logger.LogError(ctx, fmt.Sprintf("Stripe 更新邀请码订单状态失败 trade_no=%s error=%q", referenceId, err.Error()))
		}
		total, _ := strconv.ParseFloat(event.GetObjectValue("amount_total"), 64)
		currency := strings.ToUpper(event.GetObjectValue("currency"))
		logger.LogInfo(ctx, fmt.Sprintf("Stripe 邀请码购买成功 trade_no=%s amount_total=%.2f currency=%s event_type=%s client_ip=%s", referenceId, total/100, currency, string(event.Type), callerIp))
		return
	}
