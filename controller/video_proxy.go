...
func VideoProxy(c *gin.Context) {
	...
	if taskHasPluginExecution(task) {
		...
		descriptor, err = legacyVideoContentRequest(c, task)
		if err != nil {
...
// Legacy tasks keep the provider-specific credentials and URL resolution they
// used before plugins were installed, including Gemini and Vertex operations.
func legacyVideoContentRequest(c *gin.Context, task *model.Task) (*relaychannel.TaskContentRequest, error) {
	channel, err := model.CacheGetChannel(task.ChannelId)
	if err == nil {
		err = service.AuthorizeChannelForUserRequest(channel)
	}
	if err != nil {
		return nil, &taskMediaProxyError{status: http.StatusServiceUnavailable, code: "artifact_plugin_unavailable", message: "Artifact channel is unavailable", err: err}
	}
	request := &relaychannel.TaskContentRequest{URL: task.GetResultURL(), Method: c.Request.Method, Credentialless: true}
	switch channel.Type {
	case constant.ChannelTypeGemini:
		key := task.PrivateData.Key
		if key == "" {
			err = errors.New("API key not stored for task")
			break
		}
		request.URL, err = getGeminiVideoURL(channel, task, key)
		request.Headers = map[string]string{"x-goog-api-key": key}
		request.Credentialless = false
	case constant.ChannelTypeVertexAi:
		request.URL, err = getVertexVideoURL(channel, task)
	case constant.ChannelTypeOpenAI, constant.ChannelTypeSora:
		...
		request.URL = fmt.Sprintf("%s/v1/videos/%s/content", strings.TrimRight(baseURL, "/"), url.PathEscape(task.GetUpstreamTaskID()))
		request.Headers = map[string]string{"Authorization": "Bearer " + key}
		request.Credentialless = false
	}
	if err != nil {
		return nil, &taskMediaProxyError{status: http.StatusBadGateway, code: "artifact_upstream_error", message: "Failed to resolve video content", err: err}
	}
	return request, nil
}
