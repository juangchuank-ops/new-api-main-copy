...
func (*CreemAdaptor) RequestPay(c *gin.Context, req *CreemPayRequest) {
	...
	if req.ProductType == "invitation_code" {
		if req.Count < 1 || req.Count > 100 {
			c.JSON(http.StatusOK, gin.H{"message": "error", "data": "邀请码数量必须在1-100之间"})
			return
		}
	...
	if selectedProduct == nil && req.ProductType != "invitation_code" {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "产品不存在"})
		return
	}
	// For invitation codes, use the first available product for checkout (to get a valid product_id).
	if req.ProductType == "invitation_code" && selectedProduct == nil && len(products) > 0 {
		selectedProduct = &products[0]
	}
	if selectedProduct == nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "未配置有效的支付产品"})
		return
	}
	...
	productType := ""
	if req.ProductType == "invitation_code" {
		quota = int64(req.Count)
		price = common.InvitationCodePrice * float64(req.Count)
		productType = "invitation_code"
	}
	topUp := &model.TopUp{
		UserId:          id,
		Amount:          quota, // 充值额度 / 邀请码数量
		Money:           price, // 支付金额
		TradeNo:         referenceId,
	...
	checkoutUrl, err := genCreemLink(c.Request.Context(), referenceId, selectedProduct, user.Email, user.Username, units)
	if err != nil {
...
func handleCheckoutCompleted(c *gin.Context, event *CreemWebhookEvent) {
	...
	// 邀请码购买：直接生成邀请码，不充值额度
	if topUp.ProductType == "invitation_code" {
		...
		if err := topUp.Update(); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Creem 更新邀请码订单状态失败 trade_no=%s error=%q", referenceId, err.Error()))
		}
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 邀请码购买成功 trade_no=%s creem_order_id=%s count=%d client_ip=%s", referenceId, event.Object.Order.Id, topUp.Amount, c.ClientIP()))
		c.Status(http.StatusOK)
		return
	}
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string, units int) (string, error) {
