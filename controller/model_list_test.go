...
func TestListModelsKeepsRoutingAliasesWithinTokenPermissions(t *testing.T) {
	withSelfUseModeEnabled(t)
	for _, test := range []struct {
		...
		{model: "qwen3-max@thinking:on", allowed: "qwen3-max", visible: true},
		{model: "claude-3-7-sonnet-thinking", allowed: "claude-3-7-sonnet", visible: true},
		{model: "gpt-5.1-codex-max", allowed: "gpt-5.1-codex-max", visible: true},
		{model: "gpt-5.1-codex-max", allowed: "gpt-5.1", visible: false},
	} {
		t.Run(test.model+"/"+test.allowed, func(t *testing.T) {
			db := setupModelListControllerTestDB(t)
			require.NoError(t, db.Create(&model.Ability{
				Group: "default", Model: test.model, ChannelId: 1, Enabled: true,
			}).Error)
			recorder := httptest.NewRecorder()
			ctx, _ := gin.CreateTestContext(recorder)
			ctx.Request = httptest.NewRequest(http.MethodGet, "/v1/models", nil)
			common.SetContextKey(ctx, constant.ContextKeyUserGroup, "default")
			common.SetContextKey(ctx, constant.ContextKeyTokenModelLimitEnabled, true)
			common.SetContextKey(ctx, constant.ContextKeyTokenModelLimit, map[string]bool{test.allowed: true})
			ListModels(ctx, constant.ChannelTypeOpenAI)
			models := decodeListModelsResponse(t, recorder)
			_, visible := models[test.model]
