...
func TestTelegramOAuthPreservesExistingAccountAndBinding(t *testing.T) {
	...
	// Exercise the production provider's token and JWKS transport through the
	// application login-client factory, with all traffic confined to the fixture.
	oauth.RegisterLoginHTTPClientFactory(func(time.Duration) (*http.Client, error) {
		return fixture.client, nil
	})
	t.Cleanup(func() { oauth.RegisterLoginHTTPClientFactory(service.GetLoginHTTPClient) })
	oauth.Register("telegram", oauth.NewTelegramProvider(nil))
	...
	// A changed or invalid login proxy must not silently use a direct connection,
	// including after this provider has already cached its signing keys.
	oauth.RegisterLoginHTTPClientFactory(func(time.Duration) (*http.Client, error) {
		return nil, errors.New("fixture login proxy unavailable")
	})
	state, code = fixture.authorization(t, "login", service.AuthIdentity{}, "", telegramIdentityClaims(json.Number(telegramID)))
	response = telegramOAuthCallback(state, code, service.AuthIdentity{})
