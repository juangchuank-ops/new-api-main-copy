...
func RequestEpay(c *gin.Context) {
	...
	// Calculate payMoney based on product type
	...
	// Normalize amount for TopUp record
	amount := req.Amount
	if req.ProductType == "invitation_code" {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			if topUp.ProductType == "invitation_code" {
				...
				model.RecordTopupLog(topUp.UserId, fmt.Sprintf("购买邀请码成功，数量: %d，支付金额：%f", topUp.Amount, topUp.Money), c.ClientIP(), topUp.PaymentMethod, "epay")
				return
			}
			topUp.Status = common.TopUpStatusSuccess
			err := topUp.Update()
