...
func modelManagementDB(t *testing.T, kind, dsn string) *gorm.DB {
	...
	require.NoError(t, database.AutoMigrate(&model.Model{}, &model.Vendor{}, &model.Channel{}, &model.Ability{}, &model.Option{}, &model.User{}, &model.AuditLog{}, &model.NamedLease{}))
	...
	for _, dialect := range []struct{ kind, env string }{{"sqlite", ""}, {"mysql", "TEST_MYSQL_DSN"}, {"postgres", "TEST_POSTGRES_DSN"}} {
		t.Run(dialect.kind, func(t *testing.T) {
			...
			t.Run("pricing_saves_zero_switches_modes_and_rejects_stale_batches", func(t *testing.T) {
				...
				failedChange := other
				failedChange.Pricing = model.PricingValues{"ModelPrice": float64(0), "ModelRatio": float64(9)}
				err = model.UpdateModelPricing([]model.ModelPricingChange{failedChange})
				require.Error(t, err)
			...
			t.Run("metadata_preview_selection_versions_and_transaction", func(t *testing.T) {
				...
				upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					if strings.HasSuffix(r.URL.Path, "vendors.json") {
						...
						payload = []service.ModelMetadataVendor{{Name: "Matrix vendor", Status: 1}}
					} else {
						payload = []service.ModelMetadataModel{
				...
				var preview struct {
					Success bool
					Data    struct {
						Source     service.ModelMetadataCatalogSource
						Candidates []service.ModelMetadataCatalogCandidate
					}
				}
				...
				byName := make(map[string]service.ModelMetadataCatalogCandidate)
				...
				response = modelManagementRequest(t, SyncUpstreamModels, "POST", "/api/models/sync_upstream", map[string]any{"selections": []model.MetadataSyncSelection{}}, nil)
...
func TestMetadataSyncLocaleAndEndpointValidation(t *testing.T) {
	for _, locale := range []struct{ input, expected string }{{"zh", "zh"}, {"zh-CN", "zh"}, {"en", "en"}, {"ja", "ja"}} {
		t.Run(locale.input, func(t *testing.T) {
			normalized, ok := service.NormalizeModelMetadataLocale(locale.input)
			assert.True(t, ok)
			assert.Equal(t, locale.expected, normalized)
		})
	}
	_, valid := service.NormalizeModelMetadataLocale("invalid")
