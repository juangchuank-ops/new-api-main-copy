...
export type HeaderNavModule = 'rankings' | 'pricing' | 'modelAvailability'
...
export type HeaderNavModules = {
  ...
  modelAvailability: ModuleAccess
  docs: boolean
  about: boolean
...
const DEFAULT_HEADER_NAV_MODULES: HeaderNavModules = {
  ...
  modelAvailability: { enabled: true, requireAuth: false },
  docs: true,
  about: true,
}
...
const DEFAULTS: Record<HeaderNavModule, ModuleAccess> = {
  ...
  modelAvailability: DEFAULT_HEADER_NAV_MODULES.modelAvailability,
}
...
export function parseHeaderNavModules(raw: unknown): HeaderNavModules {
  ...
  Object.entries(parsed).forEach(([key, value]) => {
    ...
    if (key === 'modelAvailability') {
      result.modelAvailability = parseAccess(value, result.modelAvailability)
      return
    }
