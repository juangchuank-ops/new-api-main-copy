...
export function LogStatCards(props: LogStatCardsProps) {
  ...
  const stats = useMemo(
    () => (quotaData ? calculateDashboardStats(quotaData) : null),
    [quotaData]
  )

  // Keep parent in sync with fetched data
  useEffect(() => {
    onDataUpdate?.(quotaData ?? [], isLoading)
  }, [quotaData, isLoading, onDataUpdate])

  // Surface backend validation errors (e.g. "时间跨度不能超过 1 个月")
  // instead of silently showing empty charts.
  useEffect(() => {
