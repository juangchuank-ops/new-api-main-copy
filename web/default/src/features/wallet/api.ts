...
/** Get invitation codes owned by the current user */
export async function getUserInvitationCodes(
  page: number,
  pageSize: number
): Promise<ApiResponse<UserInvitationCodesPage>> {
  const res = await api.get(
    `/api/user/invitation-code/?p=${page}&page_size=${pageSize}`
  )
  return res.data
}

/** Get all unused invitation codes owned by the current user */
export async function getUserAvailableInvitationCodes(): Promise<
  ApiResponse<string[]>
> {
  const res = await api.get('/api/user/invitation-code/available')
  return res.data
}

/** Delete one invitation code owned by the current user */
export async function deleteUserInvitationCode(
  id: number
): Promise<ApiResponse<number>> {
  const res = await api.delete(`/api/user/invitation-code/${id}`)
  return res.data
}

/** Delete selected invitation codes owned by the current user */
export async function deleteUserInvitationCodes(
  ids: number[]
): Promise<ApiResponse<number>> {
  const res = await api.post('/api/user/invitation-code/batch-delete', { ids })
  return res.data
}

/** Delete all used invitation codes owned by the current user */
export async function deleteUserUsedInvitationCodes(): Promise<
  ApiResponse<number>
> {
  const res = await api.delete('/api/user/invitation-code/used')
  return res.data
}
