...
export function ChannelMutateDrawer({
  ...
  let apiKeyDescription: ReactNode = t(FIELD_DESCRIPTIONS.KEY)
  if (isBatchMode) {
    apiKeyDescription = t('Enter one API key per line for batch creation')
  }
  if (isEditing) {
    apiKeyDescription = (
      <>
        ...
        {isMultiKeyChannel && (
          <span className='text-warning mt-1 block'>
            ...
            {keyMode === 'replace' ? t('replaced') : t('appended')}
          </span>
        )}
      </>
    )
