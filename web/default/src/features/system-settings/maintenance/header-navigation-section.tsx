...
const headerNavSchema = z.object({
  ...
  modelAvailabilityEnabled: z.boolean(),
  modelAvailabilityRequireAuth: z.boolean(),
  docs: z.boolean(),
  about: z.boolean(),
})
...
const toFormValues = (config: HeaderNavModulesConfig): HeaderNavFormValues => ({
  ...
  modelAvailabilityEnabled:
    config.modelAvailability?.enabled === undefined
      ? HEADER_NAV_DEFAULT.modelAvailability.enabled
      : Boolean(config.modelAvailability.enabled),
  modelAvailabilityRequireAuth:
    config.modelAvailability?.requireAuth === undefined
      ? HEADER_NAV_DEFAULT.modelAvailability.requireAuth
      : Boolean(config.modelAvailability.requireAuth),
  docs:
...
export function HeaderNavigationSection({
  ...
  const onSubmit = async (values: HeaderNavFormValues) => {
    const payload: HeaderNavModulesConfig = {
      ...
      modelAvailability: {
        ...(config.modelAvailability ?? HEADER_NAV_DEFAULT.modelAvailability),
        enabled: values.modelAvailabilityEnabled,
        requireAuth: values.modelAvailabilityRequireAuth,
      },
    }
  ...
  const accessModules: Array<{
    ...
    {
      enabledKey: 'modelAvailabilityEnabled',
      requireAuthKey: 'modelAvailabilityRequireAuth',
      requireAuthDependsOn: 'modelAvailabilityEnabled',
      title: t('Model Availability'),
      description: t('Public model availability and performance metrics.'),
      requireAuthTitle: t('Require login to view model availability'),
      requireAuthDescription: t(
        'Visitors must authenticate before accessing model availability.'
      ),
    },
  ]
