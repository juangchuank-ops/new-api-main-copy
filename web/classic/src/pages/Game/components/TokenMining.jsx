...
const TokenMining = ({ t }) => {
  ...
  const MAX_SESSION_SCORE = 100000;
  ...
  return (
    <div>
      <div className='flex items-center justify-between mb-2 flex-wrap gap-2'>
        <Text strong>{t('深度')}: {depth}m · {t('得分')}: {score} · {t('矿工')}: {autoMiners}</Text>
      ...
      <div className='flex gap-2 mt-3'>
        ...
        <Button theme='solid' type='primary' onClick={hire} disabled={score < 200 * (autoMiners + 1)}>
      ...
      <Text type='secondary' className='block mt-2'>
        {t('每层产出 100+深度×10 分，每次点击 +5 分。镐子会磨损，矿工自动帮你挖。')}
      </Text>
