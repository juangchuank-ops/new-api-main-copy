...
export const useChannelsData = () => {
  ...
  const batchUpdateChannelStatus = async (status) => {
    if (selectedChannels.length === 0) {
      showError(t('请先选择要操作的通道！'));
      return;
    }
    setLoading(true);
    let ids = selectedChannels.map((channel) => channel.id);
    const res = await API.post('/api/channel/status/batch', { ids: ids, status: status });
    const { success, message, data } = res.data;
    if (success) {
      const actionText = status === 1 ? t('启用') : t('禁用');
      showSuccess(t('已${action} ${count} 个通道！').replace('${action}', actionText).replace('${count}', data));
      await refresh();
    } else {
      showError(message);
    }
  ...
  const testChannel = async (
    ...
    try {
      ...
      if (testMessage) {
        url += `&message=${encodeURIComponent(testMessage)}`;
      }
