...
export const useLogsData = () => {
  ...
  // Request body debug modal state (P2-1, root-only API)
  const [showRequestBodyModal, setShowRequestBodyModal] = useState(false);
  const [requestBodyTarget, setRequestBodyTarget] = useState(null);
  ...
  const openRequestBodyModal = (requestId) => {
    if (!requestId) return;
    setRequestBodyTarget(requestId);
  ...
  const setLogsFormat = (logs) => {
    ...
    for (let i = 0; i < logs.length; i++) {
      ...
      if (logs[i].request_id) {
        ...
        // P2-1: Request Body debug viewer (root-only API, button visible to admin)
        if (isAdminUser) {
          expandDataLocal.push({
            key: t('Request Body'),
            value: (
              <Button
                size='small'
                onClick={() => openRequestBodyModal(logs[i].request_id)}
              >
                {t('查看')}
              </Button>
            ),
  ...
  return {
    ...
    // Request body debug modal (P2-1)
    showRequestBodyModal,
    setShowRequestBodyModal,
    requestBodyTarget,
    openRequestBodyModal,
