...
const ChartsPanel = ({
  ...
  // 显式指定 theme + 透明背景，避免深色模式下图表退回白底（与默认前端一致）。
  ...
  return (
    ...
    >
      <div className='h-96 p-2'>
        ...
        {activeChartTab === '2' && (
          <VChartErrorBoundary><VChart key={`model-line-${theme}`} spec={renderSpec(spec_model_line)} option={CHART_CONFIG} /></VChartErrorBoundary>
        )}
        {activeChartTab === '3' && (
          <VChartErrorBoundary><VChart key={`pie-${theme}`} spec={renderSpec(spec_pie)} option={CHART_CONFIG} /></VChartErrorBoundary>
        )}
        {activeChartTab === '4' && (
          <VChartErrorBoundary><VChart key={`rank-bar-${theme}`} spec={renderSpec(spec_rank_bar)} option={CHART_CONFIG} /></VChartErrorBoundary>
        )}
        {activeChartTab === '5' && isAdminUser && (
          <VChartErrorBoundary><VChart key={`user-rank-${theme}`} spec={renderSpec(spec_user_rank)} option={CHART_CONFIG} /></VChartErrorBoundary>
        )}
