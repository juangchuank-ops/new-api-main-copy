...
import {
  ...
  CLIENT_IDENTITY_PROFILE_LABELS,
  CLIENT_IDENTITY_CLIENT_TYPE_FOR_PROFILE,
  CLIENT_IDENTITY_SOURCE_FOR_PROFILE,
  CLIENT_IDENTITY_SOURCE_PACKAGE_FOR_PROFILE,
  CLIENT_IDENTITY_PLATFORMS,
  allowedClientIdentityProfiles,
} from '../../../../constants';
import {
  SideSheet,
  Space,
...
const EditChannelModal = (props) => {
  ...
  const originInputs = {
    ...
    aws_key_type: 'ak_sk',
    // 客户端身份（存入 settings.client_identity，'' 表示未配置）
  ...
  // 客户端身份版本列表（来自后端 /api/channel/client-identity/versions，实时查 npm/官方源）
  const [clientIdentityVersions, setClientIdentityVersions] = useState([]);
  const [clientIdentityLatest, setClientIdentityLatest] = useState('');
  const [clientIdentityVersionsLoading, setClientIdentityVersionsLoading] =
    useState(false);
  const [clientIdentityVersionsError, setClientIdentityVersionsError] =
    useState(false);
  const [clientIdentityRefreshTick, setClientIdentityRefreshTick] =
    useState(0);
  const clientIdentityFetchSeq = useRef(0);
  // 当前生效的客户端身份：未选择或不在允许列表内时回退到渠道类型默认值
  const effectiveClientIdentityProfile = useMemo(() => {
    if (!CLIENT_IDENTITY_CHANNEL_TYPES.has(inputs.type)) {
      return 'none';
    }
    const defaults = CLIENT_IDENTITY_DEFAULTS[inputs.type];
    const profile = inputs.client_identity_profile || defaults.profile;
    const allowed = allowedClientIdentityProfiles(inputs.type);
    return allowed.includes(profile) ? profile : defaults.profile;
  }, [inputs.type, inputs.client_identity_profile]);
  const clientIdentityManualVersion =
    effectiveClientIdentityProfile !== 'none' &&
    (clientIdentityVersionsError ||
      (!clientIdentityVersionsLoading && clientIdentityVersions.length === 0));
  // npm/workbuddy 来源才有官方版本列表可刷新；manual 来源只支持手填
  const clientIdentityHasVersionList =
    effectiveClientIdentityProfile !== 'none' &&
    ['npm', 'workbuddy'].includes(
      CLIENT_IDENTITY_SOURCE_FOR_PROFILE[effectiveClientIdentityProfile],
    );
  ...
  // 按当前身份拉取官方客户端版本列表（npm / WorkBuddy 官方源），
  // manual 来源（无版本列表）与"无客户端"直接清空状态。
  useEffect(() => {
    const profile = effectiveClientIdentityProfile;
    const source = CLIENT_IDENTITY_SOURCE_FOR_PROFILE[profile] || 'manual';
    if (profile === 'none' || source === 'manual') {
      clientIdentityFetchSeq.current += 1;
      setClientIdentityVersions([]);
      setClientIdentityLatest('');
      setClientIdentityVersionsLoading(false);
      setClientIdentityVersionsError(false);
      return;
    }
    const seq = ++clientIdentityFetchSeq.current;
    setClientIdentityVersionsLoading(true);
    setClientIdentityVersionsError(false);
    API.get('/api/channel/client-identity/versions', {
      params: { profile },
    })
      .then((res) => {
        if (seq !== clientIdentityFetchSeq.current) return;
        if (res.data.success) {
          const lookup = res.data.data || {};
          setClientIdentityVersions(
            Array.isArray(lookup.versions) ? lookup.versions : [],
          );
          setClientIdentityLatest(lookup.latest || '');
        } else {
          setClientIdentityVersions([]);
          setClientIdentityLatest('');
          setClientIdentityVersionsError(true);
        }
      })
      .catch(() => {
        if (seq !== clientIdentityFetchSeq.current) return;
      ...
      .finally(() => {
        if (seq === clientIdentityFetchSeq.current) {
          setClientIdentityVersionsLoading(false);
        }
      });
  }, [effectiveClientIdentityProfile, clientIdentityRefreshTick]);

  // Codex Desktop：版本列表加载完成后，用户未选择时自动填入官方最新版本
  useEffect(() => {
    if (effectiveClientIdentityProfile !== 'codex_desktop') {
      return;
    }
    if (
      clientIdentityVersionsLoading ||
      clientIdentityVersionsError ||
      !clientIdentityLatest
    ) {
      return;
    }
    if (formApiRef.current) {
      const current = formApiRef.current.getValue('client_identity_version');
      if (current) {
        return;
      }
    ...
    setInputs((prev) =>
      prev.client_identity_version
        ? prev
        : { ...prev, client_identity_version: clientIdentityLatest },
    );
  }, [
    effectiveClientIdentityProfile,
    clientIdentityLatest,
    clientIdentityVersionsLoading,
    clientIdentityVersionsError,
  ]);
  ...
  const loadChannel = async () => {
    ...
    if (success) {
      ...
      data.client_identity_version = '';
      data.client_identity_platform = '';
      if (data.settings) {
        try {
          const parsedSettings = JSON.parse(data.settings);
          data.azure_responses_version =
            parsedSettings.azure_responses_version || '';
          // 读取客户端身份（client_identity）
          if (parsedSettings.client_identity) {
            data.client_identity_profile =
              parsedSettings.client_identity.profile || '';
            data.client_identity_version =
              parsedSettings.client_identity.version || '';
            data.client_identity_platform =
              parsedSettings.client_identity.platform || '';
          }
          // 读取 Vertex 密钥格式
  ...
  const submit = async () => {
    ...
    // 客户端身份：保存到 settings.client_identity，未选择或渠道类型不支持时移除
    if (CLIENT_IDENTITY_CHANNEL_TYPES.has(localInputs.type)) {
      const defaults = CLIENT_IDENTITY_DEFAULTS[localInputs.type];
      const allowedProfiles = allowedClientIdentityProfiles(
        localInputs.type,
      );
      let profile = localInputs.client_identity_profile || defaults.profile;
      ...
      if (profile === 'none') {
        ...
        const clientIdentity = {
          client_type: CLIENT_IDENTITY_CLIENT_TYPE_FOR_PROFILE[profile],
          profile: profile,
        };
        const version = (localInputs.client_identity_version || '').trim();
        if (version) {
          clientIdentity.version = version;
        }
        if (localInputs.client_identity_platform) {
          clientIdentity.platform = localInputs.client_identity_platform;
        }
        clientIdentity.source = {
          kind: CLIENT_IDENTITY_SOURCE_FOR_PROFILE[profile],
        };
        const sourcePackage =
          CLIENT_IDENTITY_SOURCE_PACKAGE_FOR_PROFILE[profile];
        if (sourcePackage) {
          clientIdentity.source.package = sourcePackage;
        }
        settings.client_identity = clientIdentity;
      }
              ...
              <div className='p-2 space-y-3' ref={formContainerRef}>
                ...
                <Card className='!rounded-2xl shadow-sm border-0'>
                  ...
                  <div className='flex items-center mb-4'>
                    ...
                    {CLIENT_IDENTITY_CHANNEL_TYPES.has(inputs.type) && (
                      <>
                        <Form.Select
                          field='client_identity_profile'
                          label={t('请求头（可选）')}
                          placeholder={t(
                            CLIENT_IDENTITY_PROFILE_LABELS[
                              CLIENT_IDENTITY_DEFAULTS[inputs.type].profile
                            ],
                          )}
                          optionList={allowedClientIdentityProfiles(
                            inputs.type,
                          ).map((profile) => ({
                            label: t(CLIENT_IDENTITY_PROFILE_LABELS[profile]),
                            value: profile,
                          }))}
                          style={{ width: '100%' }}
                          extraText={t(
                            '选择客户端身份后，转发到上游的请求将携带该客户端的请求头（如 User-Agent）与所选版本；选择无客户端则不添加',
                          )}
                          onChange={(value) => {
                            handleInputChange('client_identity_profile', value);
                            // 切换身份时清空版本与平台，避免残留上一个身份的取值
                            handleInputChange('client_identity_version', '');
                            handleInputChange('client_identity_platform', '');
                            if (formApiRef.current) {
                              formApiRef.current.setValue(
                                'client_identity_version',
                                '',
                          ...
                          <>
                            {clientIdentityVersionsError ? (
                              <Banner
                                ...
                                className='mb-4 rounded-xl'
                                description={t(
                                  '版本列表获取失败，可手动输入版本（格式如 0.1.2）',
                                )}
                              />
                            ) : null}
                            {clientIdentityManualVersion ? (
                              <Form.Input
                                field='client_identity_version'
                                label={t('客户端版本')}
                                placeholder={
                                  effectiveClientIdentityProfile ===
                                  'claude_desktop'
                                    ? '1.0.0'
                                    : clientIdentityLatest || '0.1.2'
                                }
                                showClear
                                extraText={
                                  effectiveClientIdentityProfile ===
                                  'claude_desktop'
                                    ? t(
                                        '填写版本后转发请求才会携带该客户端的 User-Agent（格式如 1.0.0）；留空则不添加',
                                      )
                                    : clientIdentityLatest
                                      ? `${t('官方最新版本')}：${clientIdentityLatest}`
                                      : t('留空使用后端默认版本；版本格式如 0.1.2')
                                }
                                onChange={(value) =>
                                  handleInputChange(
                                    'client_identity_version',
                              ...
                              <Form.Select
                                field='client_identity_version'
                                label={t('客户端版本')}
                                placeholder={t('使用默认版本')}
                                optionList={(() => {
                                  const options = [
                                    { label: t('使用默认版本'), value: '' },
                                  ];
                                  if (
                                    inputs.client_identity_version &&
                                    !clientIdentityVersions.includes(
                                      inputs.client_identity_version,
                                    )
                                  ) {
                                    options.push({
                                      label: `${inputs.client_identity_version} (${t('已保存')})`,
                                      value: inputs.client_identity_version,
                                    });
                                  }
                                  for (const version of clientIdentityVersions) {
                                    options.push({
                                      label: version,
                                      value: version,
                                    });
                            ...
                            ) : clientIdentityHasVersionList ? (
                              <div className='mb-2'>
                                <Button
                                  size='small'
                                  theme='light'
                                  onClick={() =>
                                    setClientIdentityRefreshTick((tick) => tick + 1)
                                  }
                                >
                                  {t('刷新版本列表')}
                                </Button>
                            ...
                            <Form.Select
                              field='client_identity_platform'
                              label={t('客户端平台')}
                              placeholder={t('使用默认平台')}
                              optionList={[
                                { label: t('使用默认平台'), value: '' },
                                ...CLIENT_IDENTITY_PLATFORMS,
                              ]}
                              style={{ width: '100%' }}
                              extraText={t(
                                '留空使用后端默认平台；选择平台后部分客户端会携带对应平台的版本构建',
                              )}
                              onChange={(value) =>
                                handleInputChange('client_identity_platform', value)
                              }
                            />
                          </>
                        )}
