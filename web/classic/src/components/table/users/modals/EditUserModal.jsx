...
const EditUserModal = (props) => {
  ...
  const getPreviewText = () => {
    ...
    const amount = adjustAmountLocal;
    const amountText = formatAmount(amount);
    const amountNum = Number(amount) || 0;
    const currentDisplay = Number(quotaToDisplayAmount(current).toFixed(6));
    let resultDisplay;
    switch (adjustMode) {
      case 'add':
        resultDisplay = currentDisplay + Math.abs(amountNum);
        return `${t('当前额度')}：${renderQuota(current)}，+${amountText} = ${resultDisplay}`;
      case 'subtract':
        resultDisplay = currentDisplay - Math.abs(amountNum);
        return `${t('当前额度')}：${renderQuota(current)}，-${amountText} = ${resultDisplay}`;
      case 'override':
        return `${t('当前额度')}：${renderQuota(current)} → ${amountText}`;
      default:
        return '';
    }
  };
