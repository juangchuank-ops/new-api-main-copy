...
func (d *diskStorage) Bytes() ([]byte, error) {
	...
	// Safety check: prevent OOM for extremely large files
	// Max 512MB for in-memory read (configurable via MAX_DISK_CACHE_BYTES_READ env)
	maxBytesRead := int64(GetEnvOrDefault("MAX_DISK_CACHE_BYTES_READ", 512*1024*1024))
	if d.size > maxBytesRead {
		return nil, fmt.Errorf("file too large for Bytes(): %d bytes exceeds max %d bytes, use streaming instead", d.size, maxBytesRead)
	}
